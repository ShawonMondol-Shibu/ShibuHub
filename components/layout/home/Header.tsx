    ...
    <Carousel className="w-full max-w-max">
      <CarouselContent className="bg-neutral-100">
        {headerData.slice(0, 5).map((item: carouselType) => {
          const { id, image, title, price, model, description, discount } = item;
          return (
            <CarouselItem key={id}>
              <div>
                <Card className="border-none shadow-none">
                  <CardContent className="flex items-center justify-center gap-10">
                    ...
                    <hgroup className="space-y-5 w-xl basis-1/3">
                      ...
                      <p className="line-clamp-3 text-lg text-neutral-700">{description}</p>
                      <p>

                        <del className="text-xl font-bold"> Discount $ {discount}</del>
                      </p>
                      <span className="flex items-center gap-5">
                        ...
                        <Button
                          variant={"outline"}
                          size={"lg"}
                          className="size-12 px-20 hover:bg-indigo-500 hover:text-white hover:shadow-2xl"
                        >
